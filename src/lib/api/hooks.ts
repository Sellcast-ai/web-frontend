 ...
 * No data at all is not a terminal status - it's the first poll, a cache
 * garbage-collected while the caller was unmounted, or a failing GET - so it
 * keeps polling too, or one failed request strands the progress card (which
 * suppresses the rest of the page) on a spinner for the page's lifetime. */
export function useImportJob(id: string) {
  return useQuery({
    ...
    refetchInterval: (query) =>
      query.state.data ? importPollInterval(query.state.data.status) : IMPORT_POLL_MS,
  });
}
