...
Deleting is scoped the same way writing is: `clearSelection` takes the store identity it is leaving and removes nothing else, so discarding a review of store B can't take the pass the user made for store A - storage holds exactly one pass, and only a gesture put it there.
This feature also attracted dead defensive branches - a `keepJob` flag, `resumeFor`'s `domain` return, two unreachable `??` fallbacks, an unresolved-user guard that `AppShell`'s session spinner made impossible - all removed rather than documented. Prefer deleting an unreachable branch here over commenting it.
