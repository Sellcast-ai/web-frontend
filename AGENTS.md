...
This feature persists client state across visits *and across accounts*: the deselection pass, the store being reviewed, and the in-flight job handle with its requested count all live in sessionStorage, which outlives a logout in the same tab (hence the `clearSelection()` next to `qc.clear()` in both logout handlers - `src/components/app/app-shell.tsx` and `src/app/app/profile/page.tsx`, deliberately still duplicated).
There is deliberately *no* client-side pre-flight guard against starting the same import twice: `POST products/import` returns the caller's existing active job instead of enqueueing a second, so a reload-then-click can't buy it twice, and a marker that outlives its own request only reintroduces the stale-restore class of bug - don't re-add one.
The two import queries pull in opposite directions on purpose: the catalog walk (`useImportCandidates`) is billed, so every automatic trigger is off and it may only start from a click; `useImportJob` is a cheap read, so it retries through a blip and only a 404 (`isJobMissing`) is treated as a dead handle - anything else keeps the handle and offers an explicit retry, because dropping a live job costs the user another walk to get back.
Anything changed here has to answer three questions, because all of this change's real bugs were found in them: what happens on a **reload**, what happens on a **logout**, and what happens to a **job that never resolves**.
