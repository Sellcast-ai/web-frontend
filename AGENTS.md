...
This feature also attracted dead defensive branches - a `keepJob` flag, `resumeFor`'s `domain` return, two unreachable `??` fallbacks, an unresolved-user guard that `AppShell`'s session spinner made impossible, `importRequested`'s no-client-count fallback - all of them defending states the feature's own cuts had already made impossible, and all removed rather than documented. Prefer deleting an unreachable branch here over commenting it.
