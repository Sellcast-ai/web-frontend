...
- `src/lib/import-selection.ts` - pure helpers for the store-import review step (sessionStorage persistence of the *deselected* set, `selectedUrls`, and `importOutcome`, which derives the finished-import toast from the counters rather than `job.status`, so a partial never reads as a clean success and a run that landed products nobody picked reads as a warning rather than a green success). The review step's shape is a settled product decision, not an accident of the diff: every candidate starts selected, one click deselects all, a live "N of M selected" count (`aria-live="polite"`) sits above the list, only the selected products are imported, and the list renders every candidate - no client page cap, no virtualization - so select-all/deselect-all can never mean "just the ones on this page". The store flow is preview -> review -> commit: `POST products/import/candidates` lists the catalog, and `POST products/import` carries `source_urls` (the reviewed subset; omit it and the backend imports everything), which it has to actually honour: the client's `requested` is the length of the `source_urls` it sent and is never taken from `products_found`, which legitimately counts the whole catalog the walk touched, so a run that lands more than was chosen surfaces as `importOvershoot` / `importIgnoredSelection` instead of being hidden. `candidates` must also return a `platform` drawn from the import adapter registry (currently `shopify` | `woocommerce`), because the client forwards it verbatim as the import's `platform` and `adapter_for()` raises -> 422 on anything outside that registry. And `candidates`' own page cap must be `>=` the import worker's cap: the review step is now the only way in, so anything the walk doesn't list can't be imported at all, and "select all" has to still mean everything the old whole-catalog import would have taken. All four are pending on the backend - the web side ships first, so a live store import 404s on candidates until that lands; the progress bar and the finished-import toast both quote the client's own count of the chosen subset, that one value and never two derivations of it, and the review step never re-enters the (billed) catalog walk without a click.
