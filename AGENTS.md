...
Those slots hold real Lumi renders (provenance in `public/marketing/videos/SOURCES.md`), so the page carries no stock-footage disclaimer; if a slot ever falls back to stock, that copy comes back with it - never let stock read as Lumi output, or Lumi output read as stock.
