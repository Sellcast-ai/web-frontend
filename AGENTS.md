...
- `src/lib/auth-redirect.ts` - server-only session-cookie presence check (`hasSession`) shared by the marketing header/pricing page, plus `redirectIfAuthenticated` for the auth pages
...
The header and footer are auth-aware (both via `hasSession`): a signed-in visitor gets "Open Studio" instead of Sign in / Start free, so no marketing chrome ever labels a link the session will just redirect away. Pricing is too (`PricingClient signedIn`): Free's CTA becomes "Open Studio" and the paid tiers open a "billing isn't self-serve yet" dialog (mailto `billing@sellcast.ai`) because there is no checkout/billing route in the frontend or backend - do not point authenticated users at `/signup?plan=` again, and do not invent a checkout.
