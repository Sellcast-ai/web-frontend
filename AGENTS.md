...
The header is auth-aware: a signed-in visitor gets a single "Open Studio" CTA instead of Sign in / Start free. Pricing is too (`PricingClient signedIn`): Free's CTA becomes "Open Studio" and the paid tiers open a "billing isn't self-serve yet" dialog (mailto `billing@sellcast.ai`) because there is no checkout/billing route in the frontend or backend - do not point authenticated users at `/signup?plan=` again, and do not invent a checkout.
`/pricing` is the *only* surface that knows whether the visitor is signed in, so it owns that decision alone: the landing page's paid-tier cards link to `/pricing` rather than to signup, and no `?plan=` param survives anywhere (nothing consumed it).
