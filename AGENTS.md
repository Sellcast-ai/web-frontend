...
- Switcher: `src/components/language-switcher.tsx` (globe dropdown, writes `lumi-locale` cookie + `router.refresh()`), mounted in the marketing `SiteFooter` (`up` prop opens the menu upward) and beside `ThemeToggle` in `AppShell`. Lists and enables all 9 target locales by endonym. This UI-language enablement is independent from `VIDEO_LANGUAGES` (the video-output axis, whose availability is backend/voice-QA controlled).
...
Real curated output footage drops in via `marketing/showcase.ts` (`HERO_OUTPUT_VIDEO`, `OUTPUT_WALL_VIDEOS`); components render designed poster placeholders while those are null - never fake customer footage.
The marketing header is deliberately slim (logo, Pricing, Sign in, one CTA; server component, no menu state); the language switcher and sign-in link live in the footer, and the theme toggle is app-only.
