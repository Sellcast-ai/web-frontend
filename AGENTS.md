...
The *only* thing persisted is the deselection pass, keyed by the current user id and the store URL it was made against, and it is read at the point of use (inside `reviewStore`, on the click) rather than at mount: `loadSelection(userId)` returns null for any other reader, so sessionStorage outliving a logout in the same tab can't offer account B a resume of account A's store review. That single read-time guard is the mechanism - it covers logout, an expired session, and every exit nobody enumerated - so don't add `clearSelection()` call sites back to the exits.
