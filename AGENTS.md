...
The *only* thing persisted is the deselection pass, keyed by the current user id and the store URL it was made against, and it is read at the point of use (`beginSelection`, inside `reviewStore`, on the click) rather than at mount: the read returns null for any other reader, so sessionStorage outliving a logout in the same tab can't offer account B a resume of account A's store review. That single read-time guard is the mechanism - it covers logout, an expired session, and every exit nobody enumerated - so don't add `clearSelection()` call sites back to the exits.
`beginSelection` is also what keeps the pass safe from its own writer: it returns null until the current user is known, and the review target it opens carries that user id, so the persist effect can only ever write under an identity a read actually succeeded for (an unresolved user used to write its empty starting set over the stored pass). For the same reason the two exits differ - `discardReview` (from a review the user has seen) drops the stored pass, `leaveWalk` (backing out of a catalog walk that never landed) leaves it alone.
