...
- [ ] Stop at the storyboard gate on that job → the hook angle / audience header appears only for the fields the storyboard actually carries (a script with neither shows no header, not empty labels), and every shot card shows both its spoken line and its visual plan, with the italic "No spoken line" / "No visual direction" placeholder where a field is blank. Both read exactly as the backend wrote them, so a truncated or reworded line is a bug, not a formatting choice
